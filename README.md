...
```bash
git clone https://github.com/hafizsafaraz/acumen.git
cd acumen
pip install .
```
...
# Argmax & argmin
c = acumen.Tensor([10, 30, 20])
print(c.argmax())  # 1
print(c.argmin())  # 0

# Abs
d = acumen.Tensor([-1, 2, -3, 4])
...
# Pow
p = acumen.Tensor([1, 2, 3, 4, 5])
print(p.pow(2))  # Tensor([1.0, 4.0, 9.0, 16.0, 25.0])

# Sqrt
s = acumen.Tensor([1, 4, 9, 16, 25])
